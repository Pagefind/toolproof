...
const REPO = "CloudCannon/toolproof";
...
async function unzipBinary(zipPath, destinationDir) {
  const expectedName = path.join(destinationDir, "toolproof");
  ...
  throw new Error(
    `Expecting toolproof or toolproof.exe unzipped into ${destinationDir}, didn't find one.`
  );
}

async function cleanCache() {
  return Promise.all(
    fs.readdirSync(cacheDir).map((dir) => {
...
module.exports = async (opts) => {
  ...
  const assetName =
    ["toolproof", opts.version, opts.target].join("-") + ".tar.gz";
  ...
  try {
    ...
    try {
      ...
      const expectedName = path.join(opts.destDir, "toolproof");
